...
class Vision(object):
    __slots__ = ('dist_to_wall', 'dist_to_apple', 'dist_to_obstacle', 'dist_to_self') ##
...
class DrawableVision(object):
    __slots__ = ('wall_location', 'apple_location', 'obstacle_location', 'self_location')##
    def __init__(self,
                wall_location: Point,
...
class Snake(Individual):
    def __init__(self, board_size: Tuple[int, int],
        ...
        num_inputs = len(self._vision_type) * 4 + 4 + 4 #@TODO: Add one-hot back in   ##
    ...
    def look_in_direction(self, slope: Slope) -> Tuple[Vision, DrawableVision]:
        ...
        vision = Vision(dist_to_wall, dist_to_apple, dist_to_obstacle, dist_to_self) ##
        drawable_vision = DrawableVision(wall_location, apple_location, obstacle_location, self_location) ##
    ...
    def _vision_as_input_array(self) -> None:
        ...
        for va_index, v_index in zip(range(0, len(self._vision) * 4, 4), range(len(self._vision))): ##adding obstacle vision
            ...
            self.vision_as_array[va_index + 2, 0] = vision.dist_to_obstacle ##
            self.vision_as_array[va_index + 3, 0] = vision.dist_to_self
            #self.vision_as_array[va_index + 2, 0] = vision.dist_to_self

        i = len(self._vision) * 4 ##
    ...
    def generate_apple(self) -> None:
        ...
        if possibilities: ##
            while(True):
                loc = self.rand_apple.choice(possibilities)
                self.apple_location = Point(loc[0], loc[1])
                if (self.apple_location != self.obstacle_location): 
    ...
    def move(self) -> bool:
        ...
        if self._is_valid(next_pos):
            ...
            # Eat the apple
                ...
                self.generate_apple()
            #elif next_pos == self.obstacle_location: ## learns not to hit wall and obstacle by not decreasing score, so should be able to learn not to hit obstacle?
